...
class VarFileProcessor(QMainWindow):
    ...
    def initUI(self):
        ...
        self.package_table.setColumnHidden(3, True)
        ...
        """)
        header_font = QFont("Roboto", 12)
        self.package_table.horizontalHeader().setFont(header_font)
        ...
        self.location_details_area = QTextEdit(central_widget)
        self.location_details_area.setReadOnly(True)
        details_font = QFont("Roboto", 10)
        self.location_details_area.setFont(details_font)
        self.location_details_area.setStyleSheet("color: #9a9a9a;")
        self.splitter.addWidget(self.location_details_area)  
        
        self.package_table.itemClicked.connect(lambda: self.display_location_details(self.package_table.currentItem()))  
        
        top_pane_layout.setContentsMargins(8, 8, 8, 4)
        package_details_layout.setContentsMargins(8, 4, 8, 4)     
        primary_splitter.setContentsMargins(0, 0, 0, 0)  
        self.log_area.setStyleSheet("QTextEdit { margin: 8px; }")
        self.location_details_area.setStyleSheet("""
            QTextEdit { 
                color: #9a9a9a; 
                margin: 8px; 
            }
    ...
    def set_default_placeholders(self):
        self.log_area.setHtml("<div style='color:#757575;'>Log</div>")
        self.location_details_area.setHtml("<div style='color:#757575;'>Location Details</div>")        
        
    def display_location_details(self, item):
        row = item.row()
        if row is not None:
            package_name_item = self.package_table.item(row, 0)
            package_name = package_name_item.text() if package_name_item else ""
            array_item = self.package_table.item(row, 3)
            if array_item:
                array_data_str = array_item.data(Qt.UserRole)
                try:
                    array_data = json.loads(array_data_str)
                    formatted_array = json.dumps(array_data, indent=2)
                    if package_name:
                        formatted_array = formatted_array.replace(package_name, f"<span style='color: #3a7cf4;'>{package_name}</span>")
                    
                    formatted_array = formatted_array.replace('\n', '<br>').replace('  ', '&nbsp;&nbsp;')
                    self.location_details_area.setHtml(formatted_array)
                except json.JSONDecodeError:
                    self.location_details_area.setPlainText("Error: Data is not valid JSON.")
            else:
                self.location_details_area.setPlainText("Error: No data found.")
    ...
    def reset_ui(self):
        self.log_area.clear()
        self.location_details_area.clear()
    ...
    def submit_files(self):
        ...
        color_package_list = "#3a7cf4"
        self.log_area.append(f"Processed packages: <span style='color:{color_package_list}'>{package_names_list}</span>\n")
    ...
    def extract_package_names(self, data, regex_pattern, package_names_info, key_names_set, key_path, filename):
        color_package_name = "#3a7cf4"
        color_filename = "#9d37a5"
        color_key_path = "#ef37ef"
                ...
                if isinstance(value, dict) or isinstance(value, list):
                    ...
                    if match:
                        ...
                        if package_name not in package_names_info:
                            ...
                            surrounding_data = self.capture_surrounding_data(data, key)
                            self.add_package_to_table(package_name, filename, key_path_str, surrounding_data)
        elif isinstance(data, list):
            for item in data:
                self.extract_package_names(item, regex_pattern, package_names_info, key_names_set, key_path, filename)
                
    def capture_surrounding_data(self, data, target_key):
        if isinstance(data, dict) and target_key in data:
            surrounding_data = json.dumps(data)
            return surrounding_data
        return "{}"
                
    def add_package_to_table(self, package_name, filename, location, surrounding_data):
        ...
        package_item.setForeground(QColor("#3a7cf4"))
        self.package_table.setItem(row_position, 0, package_item)

        filename_item = QTableWidgetItem(filename)
        filename_item.setForeground(QColor("#9a9a9a"))
        self.package_table.setItem(row_position, 1, filename_item)

        location_item = QTableWidgetItem(location)
        location_item.setForeground(QColor("#ef37ef"))
        self.package_table.setItem(row_position, 2, location_item)

        array_item = QTableWidgetItem('')
        self.package_table.setItem(row_position, 3, array_item)
        array_item.setData(Qt.UserRole, surrounding_data)
